...
export function simulateTeamRotation({ team, resolveBuild, dataset, target, passCount = 1 }) {
    ...
    for (let pass = 0; pass < passCount; pass++) {
        for (let mi = 0; mi < occupied.length; mi++) {
            ...
            if (!isFirst) {
                const introResult = simulateIntro(build, dataset, target);
            ...
            if (build.rotation?.length) {
                const simResult = simulateRotation({ build, dataset, target });
 ...
 */
function simulateIntro(build, dataset, target) {
    const introBuild = {
        ...build,
        rotation: [INTRO_KEY],
    };
    try {
        const result = simulateRotation({ build: introBuild, dataset, target });
