...
| Negative-status caps are BASE values | `NEGATIVE_STATUS_DEFS.maxStacks` is the base an enemy holds. Kits RAISE it (Xuanling S3 +3 Havoc Bane, Cartethyia S2 +3 Aero Erosion, Suisui +3 Electro Flare). Raises are not modelled, so a band above the base is reachable only via the build editor's stack stepper — do NOT "fix" a base cap to match a kit that raises it |
